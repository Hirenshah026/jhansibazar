...
<div id="screen-shopprofile" class="screen active fade-up pb-24">
    ...
    <div class="px-4 mt-6 overflow-y-auto">
        ...
        <div class="flex border-b border-ink-100 mb-4">
            ...
            <button onclick="shopTab(this,'reviews')" class="flex-1 py-2.5 text-xs font-semibold text-ink-400 hidden"
        ...
        <div id="shopTab-offers">
            <div class="space-y-3 mb-6">
                ...
                @php
                // Expiry date parse kar rahe hain
                $expiryDate = isset($offer['expiry_date']) ? \Carbon\Carbon::parse($offer['expiry_date']) : null;

                // Aaj ka din expired nahi mana jayega (isPast() check karta hai ki kya date kal ya usse purani hai)
                $isExpired = $expiryDate ? $expiryDate->isPast() && !$expiryDate->isToday() : false;
                @endphp

                <div
                    class="relative w-full {{ $isExpired ? 'bg-red-50 border-red-200' : 'bg-white border-orange-200' }} border border-dashed rounded-2xl p-4 flex items-center justify-between overflow-hidden shadow-sm hover:shadow-md transition-shadow group">

                    <div
                        class="absolute -left-3 top-1/2 -translate-y-1/2 w-6 h-6 bg-slate-100 rounded-full border-r {{ $isExpired ? 'border-red-200' : 'border-orange-200' }}">
                    </div>

                    <div class="flex items-center gap-4 pl-4">
                        <div
                            class="w-12 h-12 {{ $isExpired ? 'bg-red-200' : 'bg-orange-100' }} rounded-xl flex items-center justify-center flex-shrink-0">
                            <i data-lucide="ticket-percent"
                                class="w-6 h-6 {{ $isExpired ? 'text-red-700' : 'text-orange-600' }}"></i>
                        </div>
                        ...
                        <div>
                            ...
                            <p
                                class="text-[10px] font-bold uppercase tracking-widest mt-0.5 {{ $isExpired ? 'text-red-600' : 'text-orange-500' }}">
                                @if($expiryDate)
                                <span class="flex items-center gap-1">
                                    <i data-lucide="calendar" class="w-3 h-3"></i>
                                    {{ $isExpired ? 'Expired:' : 'Valid Till:' }} {{ $expiryDate->format('d M, Y') }}
                                </span>
                                @else
                                No Expiry Set
                                @endif
                    ...
                    <div class="pr-4 z-10 flex items-center">
                        <a href="{{ url('shop/offers') }}"
                            class="p-2 {{ $isExpired ? 'hover:bg-red-100' : 'hover:bg-slate-100' }} rounded-full transition-colors group"
                            title="Edit Offer">
                            <i data-lucide="pencil"
                                class="w-4 h-4 {{ $isExpired ? 'text-red-400 group-hover:text-red-600' : 'text-slate-400 group-hover:text-slate-600' }}"></i>
                        </a>
                    </div>

                    <div
                        class="absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-6 bg-slate-100 rounded-full border-l {{ $isExpired ? 'border-red-200' : 'border-orange-200' }}">
                    </div>
                </div>
                @empty
                <div class="w-full border-2 border-dashed border-slate-200 rounded-2xl py-8 text-center">
                    ...
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">No Active Offers Today</p>
                </div>
                @endforelse
            </div>
            ...
            <!-- Earn coins -->
            <div class="hidden bg-white border border-ink-100 rounded-2xl p-4 mb-4">
        ...
        <div id="shopTab-items" class="hidden">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                ...
                <div class="bg-white border border-ink-100 rounded-2xl overflow-hidden card-hover relative">
                    <button onclick="confirmDelete({{ $item->id }}, '{{ addslashes($item->item_name) }}')"
        ...
        <div id="shopTab-service" class="space-y-3 hidden">
            ...
            <div
                ...
                <div
                    ...
                    <img src="{{ asset('storage/' . ($service->photos ?? 'lk')) }}" class="w-full h-full object-cover"
            ...
            <div class="bg-white border border-ink-100 rounded-2xl p-4 mb-4">
                <div class="space-y-3 divide-y divide-ink-50">
                    ...
                    <div class="flex justify-between text-sm pt-3">
                        ...
                        <span class="font-medium text-ink-700">{{ implode(', ', (array) (json_decode(Session::get('shopuser')->off_days) ?? [])) }}</span>
                    </div>
                    <div class="flex justify-between text-sm pt-3">
                        <span class="text-ink-400">Category</span>
                        <span
                            class="font-medium text-ink-700">{{ implode(', ', (array) (json_decode(Session::get('shopuser')->categories) ?? [])) }}</span>
                    </div>
                    <div class="flex justify-between text-sm pt-3">
