    ...
    <div id="screen-spin" class="screen active fade-up pb-24">
        <div class="flex items-center gap-3 px-4 py-3 bg-white border-b border-ink-100 sticky top-0 z-10">
            <button onclick="window.history.back()" class="w-9 h-9 rounded-xl bg-ink-50 flex items-center justify-center">
                <svg class="w-4 h-4 text-ink-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6" /></svg>
            </button>
            <div class="flex-1">
        ...
        <div class="flex flex-col items-center px-4 pt-5">
            ...
            <button id="spinMainBtn" onclick="triggerSpin()" class="w-full gradient-brand text-white font-display font-bold text-xl rounded-2xl py-4 shadow-lg active:scale-95 transition-transform mb-4">SPIN NOW 🎡</button>

            <div class="grid grid-cols-2 gap-2.5 w-full">
                <button onclick="earnBonus('video')" class="bg-white border border-ink-200 rounded-2xl p-3 flex flex-col items-center gap-1 active:bg-slate-50">
                    <span class="text-2xl">📹</span>
                    <p class="text-xs font-bold text-ink-700">Video dekho</p>
                    <p class="text-xs text-saffron-500 font-bold">+1 extra spin</p>
                </button>
                <button onclick="earnBonus('share')" class="bg-white border border-ink-200 rounded-2xl p-3 flex flex-col items-center gap-1 active:bg-slate-50">
                    <span class="text-2xl">📤</span>
        ...
        <div id="winModal" class="hidden fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/70 backdrop-blur-sm">
            <div class="bg-white rounded-3xl w-full max-w-sm p-6 relative slide-up shadow-2xl">
                
                <button onclick="closeWin()" class="absolute -top-3 -right-3 w-10 h-10 bg-white rounded-full shadow-lg flex items-center justify-center text-gray-500 hover:text-black border border-gray-100">
                ...
                <button onclick="closeWin()" class="w-full gradient-brand text-white font-display font-bold text-base rounded-2xl py-4 shadow-lg active:scale-95 transition-transform">
                    Shukriya! 🙌
                </button>
            </div>
        </div>
...
<script>
    ...
    function drawWheel(rot) {
        ...
        if (!c) return;
        const ctx = c.getContext('2d'), cx = 145, cy = 145, r = 136, n = segments.length, arc = (2 * Math.PI) / n;
