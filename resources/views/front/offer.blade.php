                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="flex flex-wrap items-center gap-2 mt-0.5">
                                    ...
                                    @endif
                                    <span class="text-[8px] uppercase font-black {{ ($offer['is_active'] ?? true) ? 'text-emerald-500' : 'text-red-400' }}">
        ...
        <div class="bg-white w-full max-w-[380px] rounded-xl overflow-hidden shadow-2xl scale-up">
            ...
            <div class="p-5 max-h-[80vh] overflow-y-auto">
                ...
                <div class="mb-4 flex items-center gap-3">
                    <div id="img_select_box" class="w-12 h-12 border border-dashed border-slate-300 rounded-lg flex items-center justify-center bg-slate-50 cursor-pointer overflow-hidden">
                ...
                <div class="mb-3">
                    ...
                    <textarea id="offer_text_val" placeholder="e.g. Buy 1 Get 1 Free" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none focus:border-emerald-500"></textarea>
                </div>

                <div class="grid grid-cols-2 gap-3 mb-6">
                    <div>
                        <label class="text-[9px] font-black text-slate-400 uppercase ml-1">Quantity</label>
                        <input type="number" id="offer_qty_val" value="0" min="0" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none">
                    </div>
                    <div>
                        <label class="text-[9px] font-black text-slate-400 uppercase ml-1">Expiry Date</label>
                        <input type="date" id="offer_expiry_val" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none" value="{{ date('Y/m/d') }}" 
       min="{{ date('Y-m-d') }}">
                    </div>
                </div>
...
@push('script')
    <script>
        $(document).ready(function() {
            ...
            // Image Trigger
            $('#img_select_box').on('click', function() { $('#offer_img_input').click(); });
            ...
            $('#main_save_btn').on('click', function() {
                ...
                if (!text) {
                    Swal.fire({ icon: 'error', title: 'Oops', text: 'Offer text is required!' });
                    return;
                }
                alert($('#offer_qty_val').val())
                if($('#offer_qty_val').val() <= 0)
                {
                    Swal.fire({ icon: 'error', title: 'Oops', text: 'Quantity required!' });
                    return;
                }
                const $btn = $(this);
