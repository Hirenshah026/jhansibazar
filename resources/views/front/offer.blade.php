        ...
        <div class="p-3">
            <div id="offerListContainer" class="space-y-1">
                @forelse($offers as $index => $offer)
                    ...
                    <div
                        class="offer-item-card border rounded-xl p-2 px-3 flex items-center justify-between mb-2 transition-all 
                        {{ $isExpired ? 'bg-red-50 border-red-200' : 'bg-white border-slate-200' }} 
                        {{ $isActive ? '' : 'opacity-60' }}">

                        <div class="flex items-center gap-3">
                            <span
                                class="text-[10px] font-black {{ $isExpired ? 'text-red-300' : 'text-slate-300' }} w-4">{{ $index + 1 }}</span>
                            <div
                            ...
                            <div>
                                <p
                                    class="text-[11px] font-bold {{ $isExpired ? 'text-red-700' : 'text-slate-700' }} tracking-tight">
                                    {{ $offer['text'] ?? 'No Description' }}
                                    @if ($isExpired)
                                        <span class="text-[7px] text-red-500 font-black ml-1">[EXPIRED]</span>
                                    @endif
                                </p>
                                <div class="flex flex-wrap items-center gap-2 mt-0.5">
                                    <span
                                        class="text-[8px] px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded font-bold uppercase">{{ ($offer['category'] ?? 'spin') == 'spin' ? 'Spin' : 'Direct' }}</span>
                                    <span
        ...
        <div class="bg-white w-full max-w-[380px] rounded-xl overflow-hidden shadow-2xl scale-up">
            ...
            <div class="p-5 max-h-[80vh] overflow-y-auto">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="offer_text_val" maxlength="50" placeholder="e.g. 50% Cashback"
                        class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none">
                ...
                <div class="mb-3">
                    ...
                    <textarea id="offer_desc_val" rows="2" placeholder="More details..."
                        class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none"></textarea>
                </div>

                <div class="grid grid-cols-2 gap-3 mb-4">
                    <div>
                        <label class="text-[9px] font-black text-slate-400 uppercase ml-1">Quantity</label>
                        <input type="number" id="offer_qty_val" min="1"
                            class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#main_save_btn').on('click', function() {
                ...
                if (!text) {
                    $('#offer_text_val').addClass('input-error');
                    $('#err_text').show();
                    isValid = false;
                }
                if (!qty || qty <= 0) {
                    $('#offer_qty_val').addClass('input-error');
                    $('#err_qty').show();
                    isValid = false;
                }
                if (!expiry) {
                    $('#offer_expiry_val').addClass('input-error');
                    $('#err_expiry').show();
                    isValid = false;
                }
