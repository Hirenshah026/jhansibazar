...
@section('content')
        ...
        <div class="p-3">
            <div id="offerListContainer" class="space-y-1">
                @forelse($offers as $index => $offer)
                    <div
                        class="offer-item-card bg-white border border-slate-200 rounded-xl p-2 px-3 flex items-center justify-between mb-2">
                        <div class="flex items-center gap-3">
                            ...
                            <div
                                ...
                                <img src="{{ is_array($offer) && isset($offer['image']) ? $offer['image'] : asset('assets/img/gift.png') }}"
                                    class="w-full h-full object-cover"
                                    onerror="this.src='https://ui-avatars.com/api/?name={{ is_array($offer) ? $offer['text'] : $offer }}&background=random'">
                            </div>
                            <p class="text-[11px] font-bold text-slate-700 tracking-tight">
                                {{ is_array($offer) ? $offer['text'] : $offer }}</p>
                        </div>
    ...
    <div id="appModal"
        class="hidden fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-[2px] items-center justify-center px-8">
            ...
            <div class="p-5">
                ...
                <div class="mb-4 flex items-center gap-3">
                    <div id="img_select_box"
                        class="w-12 h-12 border border-dashed border-slate-300 rounded-lg flex items-center justify-center bg-slate-50 cursor-pointer overflow-hidden relative">
                    ...
                    <div class="text-[9px] text-slate-400 font-bold uppercase">Image<br><span
                            class="text-emerald-500">(Optional)</span></div>
                </div>

                <div class="mb-6">
                    <textarea id="offer_text_val" placeholder="Offer Description (e.g. 50% OFF on all items)" rows="3"
                        class="w-full px-3 py-3 bg-slate-50 border border-slate-200 rounded-lg text-lg font-bold text-slate-700 outline-none focus:border-emerald-500 transition-all resize-none"></textarea>
                </div>
                ...
                <div class="flex gap-2">
                    ...
                    <button type="button" id="main_save_btn"
                        class="flex-[1.5] py-2.5 text-[10px] font-black text-white bg-emerald-600 rounded-lg shadow-sm flex items-center justify-center gap-2 hover:bg-emerald-700 uppercase transition-all active:scale-95">
