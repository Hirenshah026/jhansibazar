...
<div class="max-w-md mx-auto p-4 space-y-6 pb-12">
    <div
        class="bg-white rounded-2xl p-5 border border-slate-300 border-t-[8px] border-t-[#1e3a8a] gov-pattern shadow-sm text-center">
        <h2 class="text-[10px] font-black text-indigo-900 uppercase tracking-widest">Marketplace Regulatory Authority
        </h2>
    ...
    <div class="bg-white rounded-2xl overflow-hidden border border-slate-300 shadow-sm">
        ...
        <div class="p-4 space-y-4">
            <div class="relative aspect-[3/1] rounded-xl overflow-hidden border-2 border-slate-200 group cursor-pointer"
                onclick="document.getElementById('banner-input').click()">
                <img id="banner-preview" src="{{ Session::get('shopuser')->banner ?? 'https://placehold.co/600x200' }}"
                    class="w-full h-full object-cover">
                <div
                    class="absolute inset-0 bg-indigo-900/20 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
                    <i data-lucide="image-plus" class="text-white w-6 h-6"></i>
                </div>
            </div>
            <div class="text-slate-500 text-sm font-medium text-center">Click box to choose logo</div>
            ...
            <form action="{{ route('shop.update.banner.local') }}" method="POST" class="ajax-form">
                ...
                <div class="flex gap-2">
                    <button type="button" onclick="location.reload()"
                        class="btn-gov flex-1 py-3 bg-slate-100 text-slate-400 rounded-xl">Discard</button>
                    <button type="submit"
                        class="btn-gov flex-[2] py-3 bg-indigo-900 text-white rounded-xl shadow-lg">Update
                        Banner</button>
                </div>
            </form>
        </div>
    ...
    <div
        class="bg-white rounded-2xl overflow-hidden border border-slate-300 shadow-sm border-b-[6px] border-b-[#f97316]">
        ...
        <div class="p-6 flex flex-col items-center space-y-5">
            <div class="relative w-28 h-28 rounded-3xl overflow-hidden border-4 border-slate-50 shadow-xl group cursor-pointer"
                onclick="document.getElementById('logo-input').click()">
                <img id="logo-preview"
                    src="{{ Session::get('shopuser')->logo ?? 'https://ui-avatars.com/api/?name=Shop' }}"
                    class="w-full h-full object-cover">

                <div
                    class="absolute inset-0 bg-indigo-900/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
                    <i data-lucide="camera" class="text-white w-6 h-6"></i>
                </div>
            </div>

            <div class="text-slate-500 text-sm font-medium">Click box to choose logo</div>
            ...
            <form action="{{ route('shop.update.logo.local') }}" method="POST" class="ajax-form w-full">
                ...
                <div class="flex gap-2">
                    <button type="button" onclick="location.reload()"
                        class="btn-gov flex-1 py-3 bg-slate-100 text-slate-400 rounded-xl">Cancel</button>
                    <button type="submit"
                        class="btn-gov flex-[2] py-3 bg-[#f97316] text-white rounded-xl shadow-lg">Save Logo</button>
                </div>
            </form>
        </div>
    </div>
...
<div id="crop-modal-overlay">
    <div id="crop-modal-content">
        <div class="px-5 py-4 border-b flex justify-between items-center">
            ...
            <button type="button" onclick="closeModal()" class="text-slate-300 hover:text-red-500"><i
                    data-lucide="x-circle"></i></button>
        </div>
        <div class="cropper-box"><img id="image-to-crop"></div>
        <div class="p-5 bg-slate-50 flex gap-3">
            <button type="button" onclick="closeModal()"
                class="btn-gov flex-1 py-3 bg-white border border-slate-200 rounded-xl">Exit</button>
            <button type="button" id="crop-apply"
                class="btn-gov flex-[2] py-3 bg-indigo-900 text-white rounded-xl">Apply</button>
        </div>
    </div>
</div>
...
<script>
    ...
    document.getElementById('crop-apply').onclick = () => {
        const canvas = cropper.getCroppedCanvas({
            width: currentType === 'logo' ? 500 : 1200
        });
    ...
    document.querySelectorAll('.ajax-form').forEach(form => {
        form.onsubmit = async (e) => {
            ...
            if (!data.get(`cropped_${currentType}`)) return Swal.fire('Error', 'Please crop first',
                'warning');
